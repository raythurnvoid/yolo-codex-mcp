...
const sessionUsageBySessionId = new Map<string, SessionUsageState>();
...
function formatResult(result: AgentTurnResult) {
	const text = `${result.text}\n\n\nsessionID, *use it to resume the conversation*: ${result.sessionId}`;
	return {
		content: [
			{
				text,
...
function sendNotification(method: string, params: JsonObject): void {
	...
	if (method === "notifications/progress") {
		...
		if (progress !== null && total !== null) {
			infoLog(
				`emit notifications/progress ${progress}/${total}${message ? ` message=${formatLogValue(message, 100)}` : ""}`,
			);
		} else if (progress !== null) {
			infoLog(`emit notifications/progress ${progress}${message ? ` message=${formatLogValue(message, 100)}` : ""}`);
		}
	}
	sendJson({
		jsonrpc: "2.0",
...
async function executeAgentToolCall(options: {
	...
	const initialSessionUsage = options.args.sessionId
		? (sessionUsageBySessionId.get(options.args.sessionId) ?? null)
		: null;
	const progress = createProgressContext({
		infoLog,
		initialUsage: initialSessionUsage,
		sendNotification,
	...
	try {
		...
		const usageState = progress.getUsageState();
		if (usageState.hasMeaningfulUsage) {
			sessionUsageBySessionId.set(result.sessionId, {
				progress: usageState.progress,
